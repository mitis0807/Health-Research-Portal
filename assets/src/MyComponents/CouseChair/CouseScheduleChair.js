...
export class UserManagement extends Component {
  ...
  handleOk = (id) => {
    ...
    fetch(`${URL.APIURL}/approveCourseByChair`, {
      method: "POST",
      headers: {
  ...
  handleForReject = (id) => {
    ...
    // marks complete proposal 
    fetch(`${URL.APIURL}/rejectCourseByChair`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    return data && data.map((item, index) => {
     console.log('item in render',item)
      return {
        ...
        signedByDean:
          !item.signedByDean ?'Pending':item.signedByDean==='approved'?'Approved':item.signedByDean==='reject'?'Rejected':item.signedByDean,
          action: [
            <React.Fragment>
                  <a
