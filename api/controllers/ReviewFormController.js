...
module.exports = {
  submitReview(req, res) {
    Logger.debug("ReviewFormController.submitReview");
    ...
    ValidateService.validate(args, (validationErr, validationErrMsgs) => {
      if (validationErr) {
        Logger.error(
          `ReviewFormController.submitReview at ValidateService.validate ${validationErr}`
        );
        console.log(
          `ReviewFormController.submitReview at ValidateService.validate ${validationErr}`
        );
        ...
        ReviewFormService.submitReview(reviewData, (err, updatedReviewData) => {
          if (err) {
            Logger.error(
              `ReviewFormController.submitReview at ReviewFormService.submitReview ${err}`
            );
            res.send({
              status: 300,
              message: sails.config.serverMessage.serverError,
            ...
            InternalUserService.getInternalUser(
              ...
              (error, foundData) => {
                if (error) {
                  Logger.error(
                    `ReviewFormController.submitReview at InternalUserService.getInternalUser ${err}`
                  );
                } else {
                  console.log(
                    `ReviewFormController.submitReview at InternalUserService.getInternalUser = ${foundData}`
                  );
                  let proposalArray = foundData.assignedProposal;
                  ...
                  InternalUserService.updateInternalUser(
                    req.session.user.id,
                    { assignedProposal: proposalArray },
                    (err1, finalData) => {
                      if (err1) {
            ...
            ApplicationService.getProposalByProposalId(
              ...
              (error, foundProposal) => {
                if (error) {
                  Logger.error(
                    `ReviewFormController.submitReview at ApplicationService.getProposalByProposalId ${error}`
                  );
                } else {
                  ...
                  ApplicationService.updateProposal(
                    ...
                    { date: new Date(), status: "Review form submitted" },
                    (err1, updatedData) => {
                      if (err1) {
                        Logger.error(
                          `ReviewFormController.submitReview at ApplicationService.updateProposal ${err1}`
                        );
                        console.log(
                          `ReviewFormController.submitReview at ApplicationService.updateProposal ${err1}`
                        );
                        ...
                        console.log(
                          `**ReviewFormController.submitReview at ApplicationService.updateProposal`
                        );
  ...
  getReviewForm(req, res) {
    Logger.debug("ReviewFormController.getReviewForm");
    let proposalId = req.body.proposalId;
    ReviewFormService.getReviewForm(proposalId, (err, reviewFormData) => {
      if (err) {
        res.send({
          status: 300,
          message: sails.config.serverMessage.serverError,
