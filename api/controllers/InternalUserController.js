...
module.exports = {
    ...
    const args = [
      {
        ...
        validations: [{ validation: "notEmpty" }, { validation: "string" }],
      },
      {
        name: "Last name",
      ...
      },
      {
        ...
        validations: [{ validation: "notEmpty" }, { validation: "email" }],
      },
    ];
    ValidateService.validate(args, (validationErr, validationErrMsgs) => {
      if (validationErr) {
        Logger.error(
          `InternalUserController.registration at ValidateService.validate ${validationErr}`
        );
        res.send({
          status: 300,
        ...
        InternalUserService.registration(registrationData, (err) => {
          if (err) {
            if (err === "Already Present") {
              res.send({
                status: 300,
                message: sails.config.serverMessage.emailAlreadyPresent,
              });
            } else {
              res.send({
                status: 300,
            ...
            res.send({
              status: 200,
              message: sails.config.serverMessage.registeredSuccess,
            });
          }
        });
      } else {
  ...
  getInternalUserWithAuth(req, res) {
    Logger.debug("InternalUserController.getInternalUserWithAuth");
    const internalUserId = req.body.internalUserId;
    console.log(internalUserId);
    if (!internalUserId) {
      res.send({
  ...
  updateInternalUser(req, res) {
   ...
   console.log('interuserdata..........',internalUserData)
    Logger.verbose(internalUserData);
  ...
  sendInvite(req, res) {
    ...
    const args = [
      ...
      },
      {
        ...
        validations: [
          ...
          },
          {
            ...
            message: sails.config.ValidationMessages.alphabetPatternValidation,
          },
          {
      ...
      },
      {
        ...
        validations: [
          {
            validation: "notEmpty",
            message: sails.config.ValidationMessages.lastNameRequiredValidation,
          },
          {
            validation: "regex",
            regex: /^[a-zA-Z-,]+(\s{0,1}[a-zA-Z-, ])*$/,
      ...
      },
      {
        ...
        validations: [
          {
            validation: "notEmpty",
            message: sails.config.ValidationMessages.roleValidation,
          },
        ],
      },
    ];
    ...
    ValidateService.validate(args, (validationErr, validationErrMsgs) => {
      if (validationErr) {
        Logger.error(
          `InternalUserController.sendInvite at ValidateService.validate ${validationErr}`
        );
        res.send({
          status: 300,
          message: sails.config.serverMessage.serverError,
        ...
        if (!internalUserData.email || !internalUserData.role) {
          res.send({
            status: 300,
            message: sails.config.serverMessage.emailRoleValidation,
          });
        } else {
          InternalUserService.sendInvite(internalUserData, (err) => {
            if (err) {
              ...
              res.send({
                status: 200,
                message: sails.config.serverMessage.inviteSuccess,
              });
            }
          });
        }
        ...
        ValidateService.getValidateMsg(
          ...
          (getValidateMsgErr, errMsgs) => {
            if (getValidateMsgErr) {
              Logger.error(
                `InternalUserController.sendInvite at ValidateService.getValidateMsg ${getValidateMsgErr}`
              );
              res.send({
                status: 300,
                message: sails.config.serverMessage.serverError,
  ...
  getAllInternalUser(req, res) {
    ...
    InternalUserService.getAllInternalUser(
      pageNo,
      limit,
      search,
      (err, internalUserData, length) => {
        if (err) {
          res.send({
            status: 300,
            message: sails.config.serverMessage.serverError,
